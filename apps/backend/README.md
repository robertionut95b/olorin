...
GraphQL Playground for the NestJS Server is available here: http://localhost:8000/graphql
...
[RESTful API](http://localhost:8000/api) documentation available with Swagger.
...
```bash
docker run -d -t -p 8000:8000 --env-file .env nest-prisma-server
```

Now open up [localhost:8000](http://localhost:8000) to verify that your nest server is running.
